...
/**
 *采用EOS去中心化技术，链上数据公开、无法篡改等特性，以及bancor算法
 *开发一个真正公平公正的游戏平台，游戏的参与者也是游戏的受益人。
 */
class eospool : public eosio::contract {
  const std::string TITLE = "THE NEW WORLD!!!";
  const uint64_t INIT_KEY = 10000000000.0;//总股本100万
  const uint64_t INIT_EOS = 100000000.0;//底仓股份1万，用于bancor定价，不能交易
  const uint64_t INIT_TIME = 1537499471; // 2018-09-21 11:11:11 启动游戏
  const uint64_t INIT_WEIGHT = 7*24*60*60; // 权重奖池领取时间（7天）
  const uint64_t INIT_BATCH = 100; // 每次分红处理人数
  const std::string TEAM_ACCOUNT = "eospoolteams";//研发团队账号

  public:
    eospool(account_name self):eosio::contract(self),users(_self, _self),games(_self, _self),bonuses(_self, _self){}

    void transfer( account_name from, account_name to, asset quantity, std::string memo ) {
		//校验账号权限
        require_auth( from );
		//校验游戏开始时间
        eosio_assert( now()>=INIT_TIME, "游戏在2018年9月21日11点11分11秒准时启动");
		//判断是主网EOS和转账金额是否正确
        if(quantity.is_valid() && quantity.symbol == S(4, EOS) && from != _self && to == _self)
        {
            ...
            if(quantity.amount==1)
            {
                ...
                action(
                	...
                	std::make_tuple(_self,from, balance, std::string("buy key :  https://eospool.tw"))
                ).send();
			//出售股份操作
			...
			//购买操作
                ...
                uint64_t fee = eos*0.1;        			               // 10% 手续费给资源消耗及研发团队
                uint64_t referrer = eos*0.10;   			           // 10% 给推荐人
                uint64_t weight = 0;     			                   // 0% 权重奖池
                uint64_t jackpot = eos*0.55;  				 		   // 55% 储备资金用于Bancor定价
				uint64_t distribute = eos-fee-referrer-weight-jackpot; // 25% 为持股用户直接分红
  ...
  private:
    ...
    typedef eosio::multi_index<N(users), user,indexed_by<N(k), const_mem_fun<user, uint64_t, &user::get_key>>> user_list;
    user_list users;
...
EOSIO_ABI_EX(eospool, (transfer)(sell)(jackpot))
